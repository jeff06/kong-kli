...
type mockHTTPClientPost struct {
	DoFunc func(req *http.Request) (*http.Response, error)
}

func (m *mockHTTPClientPost) Do(req *http.Request) (*http.Response, error) {
	return m.DoFunc(req)
}

func TestCmdPost_Run(t *testing.T) {
	...
	t.Run("Verify command initialization", func(t *testing.T) {
		...
		if consumers_sub.CmdPost.Use != "post" {
			t.Errorf("Expected command Use to be 'post', got '%s'", consumers_sub.CmdPost.Use)
		}
		if !strings.Contains(consumers_sub.CmdPost.Short, "Create a new consumer") {
			t.Errorf("Expected command Short to contain 'Create a new consumer', got '%s'", consumers_sub.CmdPost.Short)
		}
	})
}

func TestCmdPost_Init(t *testing.T) {
	// Test that the init function properly sets up the command
	cmd := consumers_sub.CmdPost

	t.Run("Required flags", func(t *testing.T) {
		// Test that the command has the required flags
		flags := []string{"custom_id", "id", "tags", "username"}
